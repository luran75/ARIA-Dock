...
def load_model_and_info(model_filename):
    with open(model_filename, "rb") as f:
        loaded_model, model_info = pickle.load(f)  # Load both the model and saved info
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage: python mycode.py model_filename mols_to_predict_on round")
        sys.exit(1)
    ...
    if external_results is not None:  # Check if file loading and prediction were successful
        output_filename = f"XGBoost_regression-grid-predicted-scores_c{chunk_num}_r{r}.csv"
