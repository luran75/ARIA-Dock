...
verbose = False  # default; overridden in __main__
...
class RandomForestOptimization:
    """
    Optuna objective wrapper for Random Forest regression.

    Cross-validation metrics for each trial are attached to the trial via
    `trial.set_user_attr`, so the metrics for the chosen best trial can be
    recovered from `study.best_trial.user_attrs` without re-running CV.
    This guarantees reported metrics correspond to the saved model.
    """
    ...
    def objective(self, trial):
        ...
        fold_results = Parallel(n_jobs=self.n_jobs)(
            delayed(evaluate_fold)(
                ...
                n_estimators, max_depth, min_samples_split, max_features,
                min_samples_leaf, bootstrap=True,
            )
        ...
        vprint(f"\nTrial {trial.number} | n_est={n_estimators} max_depth={max_depth} "
...
def train_model_with_optuna(fingerprints, docking_scores, r, n_trials=50, n_jobs=5):
    ...
    optimizer = RandomForestOptimization(fingerprints, docking_scores, n_jobs=n_jobs)
    study_name = f"rf_optuna_r{r}"
    # Seed the sampler with the same random state for reproducibility across runs
    sampler = optuna.samplers.TPESampler(seed=RANDOM_STATE)
    study = optuna.create_study(study_name=study_name, direction='minimize', sampler=sampler)

    with tqdm(total=n_trials, desc=f"Random Forest Optuna trials (round {r})") as pbar:
    ...
    best_params = dict(best_trial.params)         # the hyperparameters Optuna chose
    best_metrics = dict(best_trial.user_attrs)    # CV metrics for those exact hyperparameters

    vprint("\n" + "=" * 80)
    vprint("Final Optimization Results")
    vprint("=" * 80)
    vprint(f"\nBest Trial (#{best_trial.number}):")
    vprint(f"  n_estimators:      {best_params['n_estimators']}")
    vprint(f"  max_depth:         {best_params['max_depth']}")
    vprint(f"  min_samples_split: {best_params['min_samples_split']}")
    vprint(f"  min_samples_leaf:  {best_params['min_samples_leaf']}")
    vprint(f"  max_features:      {best_params['max_features']}")
    ...
    metrics_df.to_csv(f'RF_metrics_r{r}.csv', index=True, header=True)
    ...
    plt.savefig(f"optuna_optimization_history_r{r}_RF-regression.jpg", format="jpg")
    plt.close()
    try:
        optuna_mpl.plot_param_importances(study)
        plt.savefig(f"optuna_param_importance_RF-regression_r{r}.jpg", format="jpg")
        plt.close()
