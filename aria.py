...
weights_file = 'composite_score_weights.txt'
shutil.copy(weights_file, f'{post_processing_directory}')
...
def process_chunk(chunk_file, target_protein_file, target_ligand_file, strategy, r, cpu):
	try:
		...
		protein_file_abs = os.path.abspath(target_protein_file)
		ligand_file_abs = os.path.abspath(target_ligand_file)
		
		# Create an isolated working subdirectory for this chunk
		chunk_workdir = os.path.join(current_dir, f'workdir_{chunk_stem}')
		os.makedirs(chunk_workdir, exist_ok=True)
		
		# Copy required scripts and the chunk file into the isolated directory
		shutil.copy(f'{docking_directory}/preprocess.py', f'{chunk_workdir}/preprocess.py')
		shutil.copy(f'{docking_directory}/remove_null.py', f'{chunk_workdir}/remove_null.py')  
		shutil.copy(f'{docking_directory}/smina_progress.py', f'{chunk_workdir}/smina_progress.py')
		shutil.copy(chunk_file_abs, chunk_workdir)
		
		vprint(f'\n=== Processing {chunk_file} ===')
		
		# Run all commands from within the isolated subdirectory
		chunk_file_local = Path(chunk_file).name
		
		# 3D conversion and protonation
		vprint(f'Converting {chunk_file} to 3D...')
		result = subprocess.run(
			f'python preprocess.py {chunk_file_local} -o {chunk_stem}_3D_H.sdf -p --save-failed failed_molecules_{chunk_stem}',
			shell=True, capture_output=True, text=True, cwd=chunk_workdir)
		if result.returncode != 0:
			raise Exception(f"3D conversion failed for {chunk_file}. Error: {result.stderr}")
		
		if not os.path.exists(os.path.join(chunk_workdir, f'{chunk_stem}_3D_H.sdf')) or \
		   os.path.getsize(os.path.join(chunk_workdir, f'{chunk_stem}_3D_H.sdf')) == 0:
		...
		result = subprocess.run(
			["python", 'remove_null.py', f'{chunk_stem}_3D_H.sdf'],
			capture_output=True, text=True, cwd=chunk_workdir
		)
		if result.returncode != 0:
			raise Exception(f"Cleaning failed for {chunk_file}. Error: {result.stderr}")
		
		if not os.path.exists(os.path.join(chunk_workdir, f'{chunk_stem}_3D_H_no_nulls.sdf')):
		...
		result = subprocess.run(cmd, shell=True, capture_output=True, text=True, cwd=chunk_workdir)
		if result.returncode != 0:
			raise Exception(f"Docking failed for {chunk_file}. Error: {result.stderr}")
		
		full_docking_output = os.path.join(chunk_workdir, docking_output_file)
		if not os.path.exists(full_docking_output) or os.path.getsize(full_docking_output) == 0:
			raise Exception(f"Docking produced empty or missing output file for {chunk_file}")
		
		# Move the docking output back to the main directory
		shutil.move(full_docking_output, os.path.join(current_dir, docking_output_file))
		
		print(f'Successfully processed chunk {chunk_stem[-1]}')
		return os.path.join(current_dir, docking_output_file)
		...
		if os.path.exists(chunk_workdir):
...
boost_shap_data_directory = f'{postp_data_directory}/BoostSF-SHAP'
isExist = os.path.exists(boost_shap_data_directory)
if not isExist:
	os.makedirs(boost_shap_data_directory)	
...
shutil.copy('./preprocess.py', f'{dock_sel_mols_dir}')
...
######################################################  ML  #########################################################################
	...
	if model != 'auto':
		...
		tasks_to_run = ['rf', 'ri', 'xgb', 'knr'] # svr removed due to complications
		output_files = {}
	...
	if model == 'auto' and r == 1:
		...
		model_r2_val_dict = {'rf': r2_val_rf, 'ri': r2_val_ridge, 'xgb': r2_val_xgb, 'knr': r2_val_knr}
		model_r2_train_dict = {'rf': r2_train_rf, 'ri': r2_train_ridge, 'xgb': r2_train_xgb, 'knr': r2_train_knr}
...
###############################################################################################################################
	...
	# move the sdf file to the sdf directory
...
if not complete_docked_mols_ordered:
	...
	n_compounds = '10'
	subprocess.run(["python", 'post_processing_script.py', f'{complete_docked_mols_ordered}', '--protein', f'{target_protein_file}', '--top-n', n_compounds, '--weights', weights_file])
...
if 'boostsf-shap' in plug_in:
	...
	shutil.move(f'{boostsfshap_directory}/results-first_mol', f'{boost_shap_data_directory}/results-first_mol')
	shutil.move(f'{boostsfshap_directory}/results-second_mol', f'{boost_shap_data_directory}/results-second_mol')
	clean_directory_no_dir(boostsfshap_directory, boost_shap_data_directory)
