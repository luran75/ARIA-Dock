...
# Re-apply after all imports: medchem/datamol may reset the filter stack during import.
# The os.environ fix above handles child processes; this handles the main process.
warnings.filterwarnings('ignore', category=FutureWarning)
warnings.filterwarnings('ignore', category=DeprecationWarning, module='medchem')
warnings.filterwarnings('ignore', category=DeprecationWarning, module='datamol')
...
def calculate_compound_scores(weights_file, df: pd.DataFrame):
    # get the weights
    ...
    print(f'Using the following weights for the composite score {weights}')
    df = df.copy()
...
def run_structural_alert_filter(sdf_file: str, alerts_list: List[str], output_file: Optional[str] = None, top_n: int = 50, protein_pdb: Optional[str] = None) -> None:
    ...
    final_df = calculate_compound_scores(weights_file, final_df)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--weights", help="File containing the weights of the composite score")
