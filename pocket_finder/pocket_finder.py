...
Uses fpocket to detect binding pockets in protein structures.
Provides ranking based on volume, area, depth-proxy, and hydrophobicity.
...
FPOCKET_AVAILABLE = FPOCKET_BIN is not None
if not FPOCKET_AVAILABLE:
    print("[WARNING] fpocket not found on PATH. Install with: "
          "mamba install -c bioconda fpocket")
...
class Pocket:
    ...
    id: str                    # e.g. "KAA", "KAB", ... (kept for dashboard compat)
    name: str                  # e.g. "Pocket 1", "Pocket 2"
    volume: float              # in Å³
    area: float                # in Å²
    depth: float               # depth proxy (mean local hydrophobic density)
    hydrophobicity: float      # hydrophobicity score
    residues: List[str]        # list of residues lining the pocket
    pdb_file: Optional[str]    # path to per-pocket PDB
    color: str                 # hex color for visualization
    rank_score: float = 0.0    # computed ranking score


# ---------------------------------------------------------------------------
# Three-letter cavity ID generator: KAA, KAB, ..., KAZ, KBA, ..., KZZ
# ---------------------------------------------------------------------------
def _cavity_id(index: int) -> str:
    if index >= 26 * 26:
        # extremely unlikely, but be safe
        return f"K{index:03d}"[:3]
    a = index // 26
    b = index % 26
    return f"K{chr(ord('A') + a)}{chr(ord('A') + b)}"


# ---------------------------------------------------------------------------
# fpocket info.txt parser
# ---------------------------------------------------------------------------
# fpocket writes one block per pocket like:
#
#   Pocket 1 :
#       Score :                     0.345
#       Druggability Score :        0.821
#       Number of Alpha Spheres :   78
#       Total SASA :                412.123
#       Polar SASA :                145.678
#       Apolar SASA :               266.445
#       Volume :                    1024.567
#       Mean local hydrophobic density : 38.21
#       Mean alpha sphere radius :  3.95
#       Mean alp. sph. solvent access : 0.51
#       Apolar alpha sphere proportion : 0.62
#       Hydrophobicity score:       28.4
#       Volume score:               3.5
#       Polarity score:             5
#       Charge score :              -1
#       Proportion of polar atoms:  35.2
#       Alpha sphere density :      6.12
#       Cent. of mass - Alpha Sphere max dist: 12.4
#       Flexibility :               0.32
#
# We extract: Volume, Total SASA, Mean local hydrophobic density,
# Hydrophobicity score, Druggability Score.
# ---------------------------------------------------------------------------
_INFO_FIELD_PATTERNS = {
    "volume":                       re.compile(r"^\s*Volume\s*:\s*([-+0-9.eE]+)"),
    "total_sasa":                   re.compile(r"^\s*Total\s+SASA\s*:\s*([-+0-9.eE]+)", re.IGNORECASE),
    "mean_local_hydrophobic_density": re.compile(r"^\s*Mean\s+local\s+hydrophobic\s+density\s*:\s*([-+0-9.eE]+)", re.IGNORECASE),
    "hydrophobicity_score":         re.compile(r"^\s*Hydrophobicity\s+score\s*:\s*([-+0-9.eE]+)", re.IGNORECASE),
    "druggability_score":           re.compile(r"^\s*Druggability\s+Score\s*:\s*([-+0-9.eE]+)", re.IGNORECASE),
}
_POCKET_HEADER = re.compile(r"^\s*Pocket\s+(\d+)\s*:")


def _parse_fpocket_info(info_path: Path) -> List[Dict[str, float]]:
    """
    Parse fpocket's <stem>_info.txt into a list of dicts (one per pocket),
    in the order fpocket reports them (already ranked by druggability).
    """
    ...
    with open(info_path, "r") as fh:
        for line in fh:
            m = _POCKET_HEADER.match(line)
            if m:
                if current is not None:
                    pockets.append(current)
                current = {"pocket_index": int(m.group(1))}
                continue
            if current is None:
                continue
            for key, pat in _INFO_FIELD_PATTERNS.items():
                m2 = pat.match(line)
                if m2:
                    try:
...
# ---------------------------------------------------------------------------
# Per-pocket residue + alpha-sphere extraction
# ---------------------------------------------------------------------------
def _parse_pocket_residues(pocket_atm_pdb: Path) -> List[List[str]]:
    """
    Parse residues lining one pocket from <run>_out/pockets/pocket<N>_atm.pdb.

    Returns residues in the same shape the original fpocket code produced:
    [[resnum, chain, resname], ...]
    so the dashboard's residue formatter keeps working unchanged.
    """
    seen = set()
    residues: List[List[str]] = []
    if not pocket_atm_pdb.exists():
        return residues
    with open(pocket_atm_pdb, "r") as fh:
        for line in fh:
            ...
            try:
                resnum = str(int(line[22:26].strip()))
            except ValueError:
                continue
            ...
            if key in seen:
                continue
            seen.add(key)
            residues.append([resnum, chain, resname])
    return residues


def _parse_alpha_sphere_centers(vert_pqr: Path) -> List[Tuple[float, float, float]]:
    """
    Parse alpha-sphere centers from <run>_out/pockets/pocket<N>_vert.pqr.
    These are the "cavity grid points" the dashboard renders as spheres.
    """
    coords: List[Tuple[float, float, float]] = []
    if not vert_pqr.exists():
        return coords
    with open(vert_pqr, "r") as fh:
        for line in fh:
            if not line.startswith(("ATOM", "HETATM")):
                continue
            # PQR uses whitespace-separated fields after the fixed atom-name
            # block; the safest approach is to read coords from the same fixed
            # columns PDB uses, which fpocket respects.
            try:
                x = float(line[30:38])
                y = float(line[38:46])
                z = float(line[46:54])
            except ValueError:
                # fall back to whitespace split (some PQR writers don't pad)
                parts = line.split()
                try:
...
def _write_unified_cavity_pdb(
    out_path: Path,
    cavities: List[Tuple[str, List[Tuple[float, float, float]]]],
) -> None:
    """
    Build a single combined "cavity PDB":
    each cavity's grid points are written as HETATM records whose RESIDUE NAME
    is the cavity ID (KAA, KAB, ...). The dashboard's _parse_cavity_coordinates()
    reads exactly that, so this keeps the visualisation pipeline unchanged.
    """
    serial = 0
    with open(out_path, "w") as fh:
        fh.write("REMARK   Cavity grid points (alpha-sphere centers from fpocket)\n")
        for cav_id, coords in cavities:
            for (x, y, z) in coords:
                serial = (serial + 1) if serial < 99999 else 1
                # fixed-width PDB HETATM line; cav_id occupies columns 18-20
                fh.write(
                    f"HETATM{serial:5d}  C   {cav_id:>3s} A   1    "
...
# ---------------------------------------------------------------------------
# Main class — same public surface as before
# ---------------------------------------------------------------------------
class PocketFinder:
    """
    Finds and ranks binding pockets in protein structures using fpocket.
    """
    ...
    def __init__(self, work_dir: str = None, top_n: Optional[int] = None):
        """
        Initialize pocket finder with a working directory.

        Args:
            work_dir: Directory for fpocket runs and per-pocket PDBs.
            top_n:    Optional cap on how many top-ranked pockets to keep.
                      None (default) keeps all that pass min_volume.
        """
        if work_dir:
    ...
    def detect_pockets(self, pdb_file: str, min_volume: float = 100.0) -> Dict:
        ...
        Returns:
            Dictionary with success status and list of Pocket objects.
                {"success": bool, "pockets": [Pocket, ...],
                 "cavity_pdb": str or None, "message": str, "error": str?}
        """
        if not FPOCKET_AVAILABLE:
            return {
                "success": False,
                "error": "fpocket not found on PATH. Install with: "
                         "mamba install -c bioconda fpocket",
                "pockets": [],
            }
        ...
        try:
            # ---- 1. Run fpocket in an isolated subdir of work_dir ----
            pdb_name = Path(pdb_file).stem
            run_dir = self.work_dir / f"{pdb_name}_fpocket"
            if run_dir.exists():
                shutil.rmtree(run_dir)
            run_dir.mkdir(parents=True)

            local_pdb = run_dir / f"{pdb_name}.pdb"
            shutil.copyfile(pdb_file, local_pdb)

            cmd = [FPOCKET_BIN, "-f", str(local_pdb)]
            proc = subprocess.run(
                cmd,
                cwd=run_dir,
                capture_output=True,
            ...
            if proc.returncode != 0:
                return {
                    "success": False,
                    "error": f"fpocket failed (exit {proc.returncode}): "
                             f"{proc.stderr.strip() or proc.stdout.strip()}",
                    "pockets": [],
                }

            # ---- 2. Locate fpocket's output dir ----
            # fpocket writes <stem>_out/ next to the input pdb
            out_dir = run_dir / f"{pdb_name}_out"
            ...
            info_path = out_dir / f"{pdb_name}_info.txt"
            ...
            # ---- 3. Build Pocket objects + collect cavity coords ----
            cavities_for_pdb: List[Tuple[str, List[Tuple[float, float, float]]]] = []
            pockets: List[Pocket] = []

            for i, raw in enumerate(raw_pockets):
                volume = raw.get("volume", 0.0)
                if volume < min_volume:
                    continue

                idx = raw["pocket_index"]   # 1-based, as printed by fpocket
                cav_id = _cavity_id(i)

                area = raw.get("total_sasa", 0.0)
                # depth proxy: mean local hydrophobic density correlates with
                # buriedness; it occupies the same downstream slot fpocket's
                # avg_depth did and is what the rank score weights.
                depth = raw.get("mean_local_hydrophobic_density", 0.0)
                hydro = raw.get("hydrophobicity_score", 0.0)

                atm_pdb  = pockets_dir / f"pocket{idx}_atm.pdb"
                vert_pqr = pockets_dir / f"pocket{idx}_vert.pqr"

                residues = _parse_pocket_residues(atm_pdb)
                coords = _parse_alpha_sphere_centers(vert_pqr)
                cavities_for_pdb.append((cav_id, coords))
                ...
                pockets.append(
                    Pocket(
                        ...
                        residues=residues,
                        pdb_file=str(atm_pdb) if atm_pdb.exists() else None,
                        color=color,
                    )
                )

            # If filtering removed everything, fall back to all pockets
            if not pockets and raw_pockets:
                print(f"[POCKET] All pockets below {min_volume} Å³, returning all...")
                return self.detect_pockets(pdb_file, min_volume=0.0)

            # ---- 4. Score, rank, optional top-N truncation ----
            self._calculate_rankings(pockets)
            pockets.sort(key=lambda p: p.rank_score, reverse=True)
            if self.top_n is not None:
                pockets = pockets[: self.top_n]
                # keep cavity coords aligned with the kept pockets
                kept_ids = {p.id for p in pockets}
                cavities_for_pdb = [c for c in cavities_for_pdb if c[0] in kept_ids]

            # ---- 5. Write the unified cavity PDB the dashboard expects ----
            output_prefix = self.work_dir / pdb_name
            cavity_pdb = str(output_prefix) + "_cavities.pdb"
            _write_unified_cavity_pdb(Path(cavity_pdb), cavities_for_pdb)
            ...
            for p in pockets:
                print(f"  - {p.name}: Vol={p.volume:.1f}Å³, Area={p.area:.1f}Å², "
                      f"Depth={p.depth:.2f}, Score={p.rank_score:.2f}")
    ...
    def _calculate_rankings(self, pockets: List[Pocket]):
        ...
        # Volume (0.35), Area (0.30), Depth-proxy (0.25), Hydrophobicity (0.10)
...
def _parse_cavity_coordinates(cavity_pdb: str, pockets: List[Pocket]) -> Dict[str, List[Tuple[float, float, float]]]:
    """
    Parse atom XYZ coordinates from the  cavity grouped by pocket ID.

    fpocket encodes the cavity ID as the residue name (e.g. KAA, KAB …)
...
def create_pocket_dashboard_html(
    ...
    Args:
        ...
        cavity_pdb_path:   Path to the combined cavity PDB 
    ...
    for i, pocket in enumerate(pockets):
        ...
        for res in pocket.residues:
            if isinstance(res, (list, tuple)) and len(res) >= 3:
                # residues format is [resnum, chain, resname]
...
if __name__ == "__main__":
    print("Pocket Finder — fpocket Dashboard")
    print("=" * 50)

    parser = argparse.ArgumentParser(
        prog="pocket_finder",
        description="fpocket-based tool to identify and visualise binding pockets in a PDB file",
    ...
    if FPOCKET_AVAILABLE:
        print("✓ fpocket available")
    else:
        print("✗ fpocket not installed")
        print("  Install with: conda install -c bioconda fpocket")
