...
def evaluate_fold(train_idx, val_idx, fingerprints, docking_scores, alpha):
...
class RidgeOptimization:
    """
    Optuna objective wrapper for Ridge regression.
    ...
    def objective(self, trial):
        ...
        # Store per-fold metrics on the trial so they survive in study.best_trial
...
def train_model_with_optuna(fingerprints, docking_scores, r, n_trials=50, n_jobs=5):
    ...
    optimizer = RidgeOptimization(fingerprints, docking_scores, n_jobs=n_jobs)
    ...
    with tqdm(total=n_trials, desc=f"Ridge Optuna trials (round {r})") as pbar:
    ...
    best_params = best_trial.params       # the hyperparameters Optuna chose
    best_metrics = best_trial.user_attrs  # the CV metrics for those exact hyperparameters
    ...
    vprint("-" * 40)
    vprint(f"  Alpha:         {best_params['alpha']:.4f}")
    ...
    # Save the metrics that correspond to the saved model
    ...
    metrics_df.to_csv(f'Ridge_metrics_r{r}.csv', index=True, header=True)

    # Best model on full training data, using the best hyperparameters
    best_model = Ridge(alpha=best_params['alpha'], fit_intercept=True, solver='auto')
    ...
    history_df.to_csv(f"Ridge_optimization_history_r{r}.csv", index=False)
    vprint(f"\nOptimization history saved to 'Ridge_optimization_history_r{r}.csv'")
    ...
    plt.ylabel('MSE')
    plt.title(f'Learning Curve Round {r} - Ridge')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    ...
    plt.savefig(f"optuna_optimization_history_r{r}_Ridge-regression.jpg", format="jpg")
    plt.close()
    try:
        optuna_mpl.plot_param_importances(study)
        plt.savefig(f"optuna_param_importance_Ridge-regression_r{r}.jpg", format="jpg")
        plt.close()
    except (ValueError, RuntimeError) as e:
        vprint(f"Skipping param-importance plot ({e}); generating slice plot instead.")
        optuna_mpl.plot_slice(study)
        plt.savefig(f"optuna_slice_r{r}_Ridge-regression.jpg", format="jpg")
        plt.close()
...
if __name__ == "__main__":
    ...
    if n_samples < n_features:
        print(
            f"\n[WARNING] n_samples ({n_samples}) < n_features ({n_features}). "
            f"You are in the underdetermined regime: small alpha will yield "
            f"train MSE ~ 0 by interpolation. Trust validation metrics only, "
            f"and expect strong regularization (large alpha) to be selected."
        )
    if n_samples < 50:
        print(
            f"[WARNING] Very small training set ({n_samples} samples). "
            f"Predictive performance will likely be poor regardless of model. "
            f"Consider increasing the round-0 sampling budget."
        )
    ...
    #final_model = Ridge(alpha=best_params['alpha'], fit_intercept=True, solver='auto')
    ...
    model_info = {
        ...
        "best_cv_metrics": best_metrics,  # train/val MSE & R² for the chosen params
